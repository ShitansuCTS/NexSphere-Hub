...
const ProductInfoOne = () => {
  const { blocks, loading, fetchLocations } = useLocationStore();
    ...
    <div className="card h-100 rounded-4 overflow-hidden">
      <div className="card-body p-20">
        <div className="row g-3">
          {blocks.map((blocks) => (
            <div className="col-xl-3 col-sm-6" key={blocks.id}>
              <div className="px-24 py-16 rounded-3 border border-neutral-200 sales-card-gradient-bg-1">
                <div className="d-flex justify-content-between align-items-center gap-12">
                  <div className="flex-grow-1">
                    <h6 className="fw-semibold mb-0">{blocks.name}</h6>

                    <span className="text-secondary-light mt-1">blocks</span>
                  </div>

                  <span>
                    <Link
                      href={`/location/blocks/${blocks.id}`}
                    ...
                    <Link
                      href={`/location/blocks/edit/${blocks.id}`}
          ...
          {blocks.length === 0 && (
            <div className="col-12">
              <p className="mb-0 text-secondary-light">No blocks found</p>
            </div>
          )}
        </div>
      </div>
