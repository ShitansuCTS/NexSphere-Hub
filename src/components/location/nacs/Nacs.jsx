...
const ProductInfoOne = () => {
  const { nacs, loading, fetchLocations } = useLocationStore();
    ...
    <div className="card h-100 rounded-4 overflow-hidden">
      <div className="card-body p-20">
        <div className="row g-3">
          {nacs.map((nacs) => (
            <div className="col-xl-3 col-sm-6" key={nacs.id}>
              <div className="px-24 py-16 rounded-3 border border-neutral-200 sales-card-gradient-bg-1">
                <div className="d-flex justify-content-between align-items-center gap-12">
                  <div className="flex-grow-1">
                    <h6 className="fw-semibold mb-0">{nacs.name}</h6>

                    <span className="text-secondary-light mt-1">nac's</span>
                  </div>

                  <span>
                    <Link
                      href={`/location/nacs/${nacs.id}`}
                    ...
                    <Link
                      href={`/location/nacs/edit/${nacs.id}`}
          ...
          {nacs.length === 0 && (
            <div className="col-12">
              <p className="mb-0 text-secondary-light">No nacs found</p>
            </div>
          )}
        </div>
      </div>
