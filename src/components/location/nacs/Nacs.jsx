...
import CreateNac from "@/components/location/nacs/forms/CreateNac";
...
const ProductInfoOne = () => {
  ...
  const [editingNacId, setEditingNacId] = useState(null);
  ...
  const hasVisibleData = nacs.length > 0 || hasFetched.nacs;
  ...
  }, [fetchLocations, filters.limit, filters.page, filters.search, hasFetched.nacs, hasVisibleData]);

  const handleEditClick = useCallback((nacId) => {
    setEditingNacId(nacId);
    setTimeout(() => {
      setShowEdit(true);
    }, 50);
  }, []);
  ...
  const handleDelete = useCallback((nacId, nacName) => {
    let confirmToastId;

    const confirmDelete = async () => {
      toast.dismiss(confirmToastId);

      const response = await deleteLocation("nacs", nacId);
      if (response.success) {
        toast.success(response.message || "NAC deleted successfully");
        fetchLocations("nacs", true);
      } else {
        toast.error(response.message || "Failed to delete NAC");
      }
    };

    confirmToastId = toast(
      <div className="p-2">
        <h6 className="mb-2">Delete NAC?</h6>
        <p className="mb-3 text-secondary-light">
          Are you sure you want to delete <strong>{nacName}</strong>?
        </p>
  ...
  if (error) {
    return (
      <div className="card border-danger-subtle p-4 text-center">
        <h6 className="mb-2">Unable to load NACs</h6>
  ...
  return (
    <>
      ...
      <Offcanvas
        show={showCreate}
        title="Create NAC"
        subtitle="Provide NAC information."
        onClose={() => setShowCreate(false)}
      >
        <CreateNac onSuccess={handleCreateSuccess} />
      </Offcanvas>

      <Offcanvas
        show={showEdit}
        title="Update NAC"
        subtitle="Modify the NAC details."
        onClose={() => {
          setShowEdit(false);
          setTimeout(() => setEditingNacId(null), 300);
        }}
      >
        {editingNacId && (
          <UpdateNac nacId={editingNacId} onSuccess={handleEditSuccess} />
        )}
      </Offcanvas>
                  ...
                  <div className="card-body p-20 d-flex flex-column">
                    <div className="d-flex align-items-center gap-3 mb-20">
                      <div className="w-48-px h-48-px rounded-circle bg-primary-50 text-primary d-flex align-items-center justify-content-center flex-shrink-0">
                        <Icon icon="mdi:map-marker-radius-outline" className="text-xl" />
                      </div>

                      <div className="flex-grow-1 min-w-0">
                        <h6 className="mb-0 fw-semibold text-truncate">{nac.name}</h6>
                        <span className="text-secondary-light small">
                          {nac.district?.name || "Active"}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-16 border-top">
                      <div className="d-flex align-items-center justify-content-between">
                        <small className="text-secondary-light d-flex align-items-center gap-1">
                          ...
                          {new Date(nac.createdAt).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                            year: "numeric",
                          })}
                        ...
                        <div className="d-flex align-items-center gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => handleEditClick(nac.id)}
                          ...
                          <button
                            type="button"
                            onClick={() => handleDelete(nac.id, nac.name)}
            ...
            {nacs.length === 0 && (
              <div className="col-12">
                <div className="card border text-center py-5">
                  <Icon icon="mdi:map-outline" className="text-secondary-light text-4xl mb-3" />
