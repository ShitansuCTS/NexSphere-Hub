...
const ProductInfoOne = () => {
  const { wards, loading, fetchLocations } = useLocationStore();
    ...
    <div className="card h-100 rounded-4 overflow-hidden">
      <div className="card-body p-20">
        <div className="row g-3">
          {wards.map((wards) => (
            <div className="col-xl-3 col-sm-6" key={wards.id}>
              <div className="px-24 py-16 rounded-3 border border-neutral-200 sales-card-gradient-bg-1">
                <div className="d-flex justify-content-between align-items-center gap-12">
                  <div className="flex-grow-1">
                    <h6 className="fw-semibold mb-0">{wards.name}</h6>

                    <span className="text-secondary-light mt-1">wards</span>
                  </div>

                  <span>
                    <Link
                      href={`/location/wards/${wards.id}`}
                    ...
                    <Link
                      href={`/location/wards/edit/${wards.id}`}
          ...
          {wards.length === 0 && (
            <div className="col-12">
              <p className="mb-0 text-secondary-light">No wards found</p>
            </div>
          )}
        </div>
      </div>
